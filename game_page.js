...
function send() {
    ...
    littleBoy = Math.floor(word.length/2);
    ceCot = word.charAt(littleBoy);

    elSalvador = word.length - 1;
    cocacolasafe = word.charAt(elSalvador);

    topSecret1 = word.replace(dowJones , "_");
    topSecret2 = topSecret1.replace(ceCot, "_");
    topSecret3 = topSecret2.replace(cocacolasafe, "_");
